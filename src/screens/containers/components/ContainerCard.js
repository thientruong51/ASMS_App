...
export default function ContainerCard({ item = {}, onInfo, onConfig, onStart, onPress }) {
  ...
  const handlePress = () => {
    ...
    navigation.navigate("OrderDetail", { orderId: item.orderCode ?? item.ref ?? item.id ?? "unknown", order: item });
  };
        ...
        <Card.Content>
          ...
          <View style={[styles.topDetailRow, { marginTop: 10 }]}>
            ...
            <View style={styles.detailCol}>
              ...
              <Text
                style={[
                  styles.smallValue,
                  { color: Number(item.unpaidAmount) > 0 ? "#e74c3c" : "#1AA64B", fontWeight: "700" },
                ]}
              >
          ...
          <View style={styles.metaRow}>
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              <Text style={[styles.paymentStatus, { color: statusColor(item.paymentStatus) }]}>{mapPaymentToVN(item.paymentStatus)}</Text>
            </View>
          </View>
